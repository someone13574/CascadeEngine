...
{
	XCB_Window::XCB_Window(std::string window_title, uint32_t window_width, uint32_t window_height, Cascade_Graphics::Graphics** graphics_ptr, Thread_Manager* thread_manager_ptr, Cascade_Graphics::Graphics_Factory* m_graphics_factory_ptr) :
		Window::Window(window_title, window_width, window_height, graphics_ptr, thread_manager_ptr)
	{
		...
		m_window_thread_ptr->Await_State(Thread::Thread_State::LOOP_FUNC);
	...
	void XCB_Window::Create_Window()
	{
		...
		m_xcb_request_check_cookie = xcb_create_window_checked(m_xcb_connection_ptr, XCB_COPY_FROM_PARENT, m_xcb_window, m_xcb_screen_ptr->root, 0, 0, m_window_width, m_window_height, 10, XCB_WINDOW_CLASS_INPUT_OUTPUT, m_xcb_screen_ptr->root_visual, XCB_CW_BACK_PIXEL | XCB_CW_EVENT_MASK, values);
