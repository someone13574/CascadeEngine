...
namespace Cascade_Graphics
{
    namespace Vulkan
    {
        ...
        void Instance_Builder::Ensure_All_Required_Layers_Are_Present()
        {
            ...
            {
                LOG_FATAL << "Graphics (Vulkan): Failed to get number of available instance layers with VkResult " << string_VkResult(get_available_layer_count_result);
                exit(EXIT_FAILURE);
            }
            ...
            {
                LOG_FATAL << "Graphics (Vulkan): Failed to get available instance layers with VkResult " << string_VkResult(get_available_layers_result);
                exit(EXIT_FAILURE);
            }
        ...
        void Instance_Builder::Ensure_All_Required_Extensions_Are_Present()
        {
            ...
            for (uint32_t layer_index = 0; layer_index <= m_enabled_layers.size(); layer_index++)
            {
                ...
                {
                    LOG_FATAL << "Graphics (Vulkan): Failed to get number of available instance extensions in layer '" << layer_name << "' with VkResult " << string_VkResult(get_extension_count_in_layer_result);
                    exit(EXIT_FAILURE);
                }
                ...
                {
                    LOG_FATAL << "Graphics (Vulkan): Failed to get available instance extensions in layer '" << layer_name << "' with VkResult " << string_VkResult(get_extension_count_in_layer_result)
                              << ")";
                    exit(EXIT_FAILURE);
                }
