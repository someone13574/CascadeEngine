...
namespace Cascade_Graphics
{
    namespace Vulkan_Backend
    {
        ...
        {
            LOG_TRACE << "Vulkan Backend: Checking if physical device has required queues";
            ...
            LOG_TRACE << "Vulkan Backend: Found " << queue_family_count << " queue_families";
            ...
            {
                LOG_TRACE << "Vulkan Backend: Has all required queues";
                return true;
            }
            else
            {
                LOG_TRACE << "Vulkan Backend: Does not have all required queues";
                return false;
            }
        }
        ...
        {
            LOG_INFO << "Vulkan Backend: Getting queue families";
            ...
            LOG_DEBUG << "Vulkan Backend: Found " << queue_family_count << " queue_families";
            ...
            if (m_required_queues & Queue_Types::GRAPHICS_QUEUE)
            {
                ...
                {
                    LOG_DEBUG << "Vulkan Backend: Graphics queue family set to index " << m_queue_family_indices.graphics_family_index.value();
                }
                else
                {
                    LOG_ERROR << "Vulkan Backend: Selected physical device does not have a graphics queue";
                    exit(EXIT_FAILURE);
                }
            }
            if (m_required_queues & Queue_Types::COMPUTE_QUEUE)
            {
                ...
                {
                    LOG_DEBUG << "Vulkan Backend: Compute queue family set to index " << m_queue_family_indices.compute_family_index.value();
                }
                else
                {
                    LOG_ERROR << "Vulkan Backend: Selected physical device does not have a compute queue";
                    exit(EXIT_FAILURE);
                }
            }
            if (m_required_queues & Queue_Types::TRANSFER_QUEUE)
            {
                ...
                {
                    LOG_DEBUG << "Vulkan Backend: Transfer queue family set to index " << m_queue_family_indices.transfer_family_index.value();
                }
                else
                {
                    LOG_ERROR << "Vulkan Backend: Selected physical device does not have a transfer queue";
                    exit(EXIT_FAILURE);
                }
            }
            if (m_required_queues & Queue_Types::SPARSE_BINDING_QUEUE)
            {
                ...
                {
                    LOG_DEBUG << "Vulkan Backend: Sparse binding queue family set to index " << m_queue_family_indices.sparse_binding_family_index.value();
                }
                else
                {
                    LOG_ERROR << "Vulkan Backend: Selected physical device does not have a sparse binding queue";
                    exit(EXIT_FAILURE);
                }
            }
            if (m_required_queues & Queue_Types::PROTECTED_QUEUE)
            {
                ...
                {
                    LOG_DEBUG << "Vulkan Backend: Protected queue family set to index " << m_queue_family_indices.protected_family_index.value();
                }
                else
                {
                    LOG_ERROR << "Vulkan Backend: Selected physical device does not have a protected queue";
                    exit(EXIT_FAILURE);
                }
            }
            if (m_required_queues & Queue_Types::PRESENT_QUEUE)
            {
                ...
                {
                    LOG_DEBUG << "Vulkan Backend: Present queue family set to index " << m_queue_family_indices.present_family_index.value();
                }
                else
                {
                    LOG_ERROR << "Vulkan Backend: Selected physical device does not have a present queue";
                    exit(EXIT_FAILURE);
                }
            }

            LOG_TRACE << "Vulkan Backend: Finished getting queue families";
        }

        void Queue_Manager::Get_Device_Queue_Handles(VkDevice* device_ptr)
        {
            LOG_INFO << "Vulkan Backend: Getting device queues from logical device";
        ...
        VkQueue* Queue_Manager::Get_Queue(Queue_Types queue_type)
        {
            ...
            {
                LOG_ERROR << "Vulkan Backend: The requested queue type was not required";
                exit(EXIT_FAILURE);
            }
        }
        ...
        uint32_t Queue_Manager::Get_Queue_Family_Index(Queue_Types queue_type)
        {
            if (m_required_queues & queue_type)
            {
                ...
                LOG_ERROR << "Vulkan Backend: Queue type does not exist";
                exit(EXIT_FAILURE);
            }
            else
            {
