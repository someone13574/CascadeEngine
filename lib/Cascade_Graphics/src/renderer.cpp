...
namespace Cascade_Graphics
{
    ...
    void Renderer::Initialize_Vulkan()
    {
        ...
        m_instance_ptr = std::make_shared<Vulkan::Instance>("Application name", 0, required_instance_extensions);
        ...
        m_physical_device_ptr = std::make_shared<Vulkan::Physical_Device>(m_instance_ptr, m_queue_manager_ptr, m_surface_ptr, required_device_extensions);
        ...
        m_storage_manager_ptr->Create_Resource_Grouping(
            ...
            {{"render_target", 0, Vulkan::Resource_ID::IMAGE_RESOURCE}, {"camera_data", 0, Vulkan::Resource_ID::BUFFER_RESOURCE}, {"voxel_buffer", 0, Vulkan::Resource_ID::BUFFER_RESOURCE}, {"hit_buffer", 0, Vulkan::Resource_ID::BUFFER_RESOURCE}});

        m_storage_manager_ptr->Create_Resource_Grouping("swapchain_images", {{"swapchain", 0, Vulkan::Resource_ID::IMAGE_RESOURCE}, {"swapchain", 1, Vulkan::Resource_ID::IMAGE_RESOURCE}, {"swapchain", 2, Vulkan::Resource_ID::IMAGE_RESOURCE}});
    ...
    void Renderer::Record_Vulkan_Command_Buffers(unsigned int width, unsigned int height)
    {
        if (m_command_buffer_manager_ptr != nullptr)
        {
            for (unsigned int i = 0; i < m_swapchain_ptr->Get_Swapchain_Image_Count(); i++)
            {
                ...
                m_command_buffer_manager_ptr->Image_Memory_Barrier({"render_frame", i}, {"swapchain", i, Vulkan::Resource_ID::IMAGE_RESOURCE}, VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR, VK_PIPELINE_STAGE_TRANSFER_BIT);
    ...
    void Renderer::Recreate_Swapchain()
    {
        if (m_renderer_initialized)
        {
            ...
            m_storage_manager_ptr->Create_Resource_Grouping("per_frame_descriptors", {{"render_target", 0, Vulkan::Resource_ID::IMAGE_RESOURCE},
                                                                                      {"camera_data", 0, Vulkan::Resource_ID::BUFFER_RESOURCE},
                                                                                      {"voxel_buffer", 0, Vulkan::Resource_ID::BUFFER_RESOURCE},
                                                                                      {"hit_buffer", 0, Vulkan::Resource_ID::BUFFER_RESOURCE}});
            ...
            m_storage_manager_ptr->Upload_To_Buffer_Staging({"voxel_buffer", 0, Vulkan::Resource_ID::BUFFER_RESOURCE}, {"staging_buffer", 0, Vulkan::Resource_ID::BUFFER_RESOURCE}, gpu_voxels.data(),
    ...
    void Renderer::Render_Frame()
    {
        ...
        if (m_renderer_initialized && m_rendering_active)
        {
            ...
            VkResult queue_present_result = vkQueuePresentKHR(*m_queue_manager_ptr->Get_Queue(Vulkan::Queue_Manager::Queue_Types::PRESENT_QUEUE), &present_info);
