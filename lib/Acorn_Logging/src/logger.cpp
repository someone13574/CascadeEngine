...
{
	Log_Stream::Log_Stream(Logger* logger_ptr, Severity_Level severity_level, unsigned int occurrence_line, std::string occurrence_file) :
		m_logger_ptr(logger_ptr), log_message_info({})
	{
	...
	void Logger::Processing_Loop(Logger* logger_ptr)
	{
		...
		while (logger_ptr->m_loop_active)
		{
			...
			logger_ptr->m_thread_notify.wait(queue_lock, [logger_ptr]
											 { return !logger_ptr->m_loop_active || !logger_ptr->m_message_queue.empty(); });
		}
	}
