...
void Update(Cascade_Core::Application* instance_ptr)
{
    ...
    camera_ptr->Set_Position({std::sin(elapsed_seconds) * 3.0, std::sin(elapsed_seconds * 5.0) * 0.5 + 1, std::cos(elapsed_seconds) * 3.0});
...
int main()
{
    ...
    main_window_ptr->Get_Renderer()->Get_Object_Manager()->Create_Object_Volume_Function("test", 6, Cascade_Graphics::Vector_3<double>(0, 0, 0), 2, Volume_Sample_Function);
