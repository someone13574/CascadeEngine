...
{
    double elapsed_seconds = instance_ptr->Get_Elapsed_Time().count() / 1000.0 / 20.0;

    std::shared_ptr<Cascade_Graphics::Camera> camera_ptr = main_window_ptr->Get_Renderer()->Get_Camera();

    camera_ptr->Set_Position({std::sin(elapsed_seconds) * 2, std::sin(elapsed_seconds * 3.0) * 2, std::cos(elapsed_seconds) * 2});
    camera_ptr->Look_At(Cascade_Graphics::Vector_3<double>(std::sin(elapsed_seconds + 0.1) * 2, std::sin((elapsed_seconds + 0.1) * 3.0) * 2, std::cos(elapsed_seconds + 0.1) * 2) * 0.75);
...
double Volume_Sample_Function(Cascade_Graphics::Vector_3<double> position)
{
    ...
    return position_length - 1.8 + (std::sin(position.m_x * 10.0) * std::sin(position.m_y * 10.0) * std::sin(position.m_z * 10.0) * 0.2) + (std::sin(position.m_x * 20.0) * std::sin(position.m_y * 20.0) * std::sin(position.m_z * 20.0) * 0.1);
}
...
int main()
{
    ...
    main_window_ptr->Get_Renderer()->Get_Object_Manager()->Create_Object_From_Volume_Function("test", 11, Cascade_Graphics::Vector_3<double>(0, 0, 0), 2.0, Volume_Sample_Function, Color_Sample_Function);
