...
void Update(Cascade_Core::Application* instance_ptr)
{
    ...
    camera_ptr->Set_Position({std::sin(elapsed_seconds) * 2.2, 0, std::cos(elapsed_seconds) * 2.2});
    camera_ptr->Look_At({0, 0, 0});
}

double Volume_Sample_Function(Cascade_Graphics::Vector_3<double> position)
{
    return position.Length() - 1.8 + (std::sin(position.m_x * 10.0) * std::sin(position.m_y * 10.0) * std::sin(position.m_z * 10.0) * 0.1);
}
...
int main()
{
    ...
    main_window_ptr->Get_Renderer()->Get_Object_Manager()->Create_Object_From_Volume_Function("test", 10, Cascade_Graphics::Vector_3<double>(0, 0, 0), 2, Volume_Sample_Function);
